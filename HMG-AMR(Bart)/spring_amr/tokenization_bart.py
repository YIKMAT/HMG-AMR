...
class AMRBartTokenizer(BartTokenizer):
    ...
    def __init__(self, *args, use_pointer_tokens=False, collapse_name_ops=False, **kwargs):
        ...
        self.modified = 0
        self.mg_list = kwargs['config'].mg_list
    ...
    # 自己加：
    def batch_encode_sentences_multi(self, sentences, mg_phrase_1_len, mg_phrase_2_len, mg_phrase_3_len, mg_sentence_len, mg_subsentence_len, mg_sentences, device=torch.device('cpu')):
        sentences = [s for s in sentences]
        extra = {'sentences': sentences}

        # create sentences by mg_sentences ,为了匹配分割句子时的问题
        new_sentences = []
        for mg_sentence in mg_sentences:
            new_sent = ""
            for sentence in mg_sentence:
                new_sent = new_sent + " " + sentence
            new_sentences.append(new_sent.strip())
        # batch = super().batch_encode_plus(sentences, return_tensors='pt', pad_to_max_length=True)
        batch = super().batch_encode_plus(new_sentences, return_tensors='pt', pad_to_max_length=True)
        batch = {k: v.to(device) for k, v in batch.items()}

        # -------------核对单词数和句子匹配
        for index, sentence_len in enumerate(mg_sentence_len):
            assert int(batch['attention_mask'][index].sum()) == sum(sentence_len)

        # ==================在这里设置相关的配置==========================
        mg_list = [int(num) for num in self.mg_list.split("-")]
        # =========短语=========
        if mg_list[0]!=0:
            batch["mg_phrase_1_len"] = mg_phrase_1_len
        if mg_list[1]!=0:
            batch["mg_phrase_2_len"] = mg_phrase_2_len
        if mg_list[2]!=0:
            batch["mg_phrase_3_len"] = mg_phrase_3_len
        # batch["mg_phrase_3_len"] = mg_phrase_3_len
        # =========子句========
        if mg_list[3]!=0:
            batch["mg_subsentence_len"] = mg_subsentence_len
        # =======句子=====这个一定要放到最后
        batch["mg_sentence_len"] = mg_sentence_len

        return batch, extra


    # 自己加：带着输入的句子和phrase列表，tokenize
    def batch_encode_sentences_phrase(self, sentences,phrase_list, phrase_list_len,  device=torch.device('cpu')):
        sentences = [s for s in sentences]
        extra = {'sentences': sentences}
        batch = super().batch_encode_plus(sentences, return_tensors='pt', pad_to_max_length=True)
        batch = {k: v.to(device) for k, v in batch.items()}
        ...
        # ---------处理phrase-------
        # (1) 将batch内的sample，都从子词对应成相应的id
        phrase_batch = []
        phrase_len_batch = []
        for index, phrase in enumerate(phrase_list):
            phrases_tokenized = self.get_phrases_tokenized(phrase)
            phrase_batch.append(phrases_tokenized)
            phrase_len_batch.append(phrase_list_len[index])

        # （2）#PAD phrase and phrase_len。将一个batch内的短语长度，都对齐成一样的。
        self.pad_all_(phrase_len_batch, padding_element=0) #0 is padding element for lengths
        self.pad_all_(phrase_batch, padding_element=self.encoder['Ġ<pad>']) #这里是1
        assert len(phrase_len_batch[0]) == len(phrase_batch[0])

        # （3）將phrase变成tensor
        batch["phrase_batch"] = torch.tensor(phrase_batch)
        batch["phrase_len_batch"] = torch.tensor(phrase_len_batch)
        # phrase_attention_mask = phrase_len_batch.to(next(self.parameters()).device) == 0
        # batch["attention_mask"] = torch.tensor(phrase_len_batch).to(device) != 0

        return batch, extra

    # 自己加：根据输入phrase的子词，返回相应的id。
    def get_phrases_tokenized(self, phrase_list):
        '''
        Returns a list of phrases tokenized by vocab
        '''
        phrase_id = []
        for phrase in phrase_list:
            phrase_id.append([self.encoder[token] for token in phrase])
        return phrase_id

    # 自己加：给同一个batch内的phrase补齐成一样的长度。
    def pad_all_(self, data, padding_element=None):
        S = deque()  # stack
        Q = deque()  # queue
        Q.append((data, 0))
        cur_dim = 0
