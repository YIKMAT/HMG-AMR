...
from utils.stanza_constituent import GENERATE_CONSTITUENT
...
def do_train(checkpoint=None, direction='amr', split_both_decoder=False, fp16=False):
    ...
    # ===使用stanza生成constituent文件===
    # input_file_dir = "/sda/yikmat/PycharmCode/Hierar_AMR/Mul_AMR/data/AMR/min/train_min_sentence.txt"
    # pickle_dir = "/sda/yikmat/PycharmCode/Hierar_AMR/Mul_AMR/data/AMR/min/train_min_tree_new.pickle"
    # GENERATE_CONSTITUENT(tokenizer, input_file_dir, pickle_dir)
    ...
    dev_loader = instantiate_loader(
        ...
        p_mg = config['p_mg'],
        constituent_file=config['dev_constituent'],
        sentence_file = config['dev_sentence'],
        clause_file=config['dev_clause']
    )
    ...
    def log_trn_loss(engine):
        ...
        print(log_msg)
    #  =========（3）Train完所有batch后，调用这个函数，然后调用 def eval_step(engine, batch):============
    @trainer.on(Events.EPOCH_COMPLETED)
    ...
    if not config['best_loss']:
        if direction in ('amr', 'both'):
            ...
            def smatch_eval(engine):
                ...
                #调用evaluation.py的predict_amrs函数
    ...
    if config['save_checkpoints']:
        ...
        CHECKPOINT_DIR = where_checkpoints
    ...
    def record_time(text):
        filename = CHECKPOINT_DIR +'/record_time.txt'
        with open(filename, 'a') as file_object:
            file_object.write(text+"\n")

    def record_dev_smatch(text):
        filename = CHECKPOINT_DIR +'/record_dev_smatch.txt'
        with open(filename, 'a') as file_object:
            file_object.write(text+"\n")

    # ======================CL===========================
    if config['IS_IC_Train']:
        ...
        p_mg = config['p_mg'],
            ...
            constituent_file=config['train_constituent'],
            sentence_file=config['train_sentence'],
            clause_file=config['train_clause']
        )
