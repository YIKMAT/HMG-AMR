...
from amr_parsing.t5.models.modeling_t5_amr import  T5ForConditionalGeneration
...
class LG(torch.nn.Module, ABC):
    def __init__(self,
                 ...
                 model_type='t5',
                 head_list=None,
                 mg_layer_begin_index=None,
                 mg_layer_end_index=None,
                 mg_list=None,
                 hie_layer_begin_index=None,
                 hie_layer_end_index=None,
                 hie_mode=None,
                 lstm_state=None
                 ):
        super().__init__()
        ...
        if model_type == 'bart':
            ...
            config.head_list = head_list
            config.mg_layer_begin_index = mg_layer_begin_index
            config.mg_layer_end_index = mg_layer_end_index
            config.mg_list = mg_list
            config.hie_layer_begin_index = hie_layer_begin_index
            config.hie_layer_end_index = hie_layer_end_index
            config.hie_mode = hie_mode
            config.lstm_state = lstm_state
            self.tokenizer = T5Tokenizer.from_pretrained(model_name,config=config)
    ...
    def forward(self, batch_source, batch_target, index, mg_dict, mg_list, ret_batch_loss='loss'):
        """
        Forward function
        ...
        """
        source_ids, source_mask, mg_dict_batch = prepare_data(batch_source, self.tokenizer, self.max_source_length, index, mg_dict, mg_list)
        target_ids, target_mask, _ = prepare_data(batch_target, self.tokenizer, self.max_target_length, self.target_pad_id, None, None, None)
    ...
    def generate(self, texts, index, mg_dict, mg_list, max_decoder_batch_size=256, num_beams=4, num_ret_seq=1):
        start_idx = 0
        results = []
        for j in range(len(texts)):
            if j % max_decoder_batch_size == max_decoder_batch_size - 1 or j == len(texts) - 1:
                end_idx = j + 1
                input_ids, attention_mask, mg_dict_batch = prepare_data(texts[start_idx:end_idx], self.tokenizer, self.max_source_length, index, mg_dict, mg_list)
                # Generate
